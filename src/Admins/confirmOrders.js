...
useEffect(()=>{
	axios.get(`https://app-ndoejs.herokuapp.com/order/${idOrder}`)
...
const handleSubmit = (e) => {
  ...
  const Orderupdate = {status : id};
    axios.put(`https://app-ndoejs.herokuapp.com/order/update/${idOrder}`,Orderupdate)
    .then(res => {
	if(res.error){
	  return false
	}else{
  ...
  const emaildata = {email};
    axios.post(`https://app-ndoejs.herokuapp.com/send_mail`,emaildata)
    .then(res => {
        if(res.error){
          return false
        }
